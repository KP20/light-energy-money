...
(function () {
    ...
    $scope.lumens_options =[375,600,900,1175,1600];
    ...
    $scope.calculate = function()
    {
     ...
     $scope.led_wattage = ($scope.current_lumens*$scope.led_conversion).toFixed(1);
        ...
        var hours = ($scope.current_hours*$scope.total_days);
        var cost = $scope.current_cost/100;
        $scope.inc_cost = ((($scope.inc_wattage*hours)/1000)*cost).toFixed(2);
        $scope.hal_cost = ((($scope.hal_wattage*hours)/1000)*cost).toFixed(2);
        $scope.cfl_cost = ((($scope.cfl_wattage*hours)/1000)*cost).toFixed(2);
        $scope.led_cost = ((($scope.led_wattage*hours)/1000)*cost).toFixed(2);
